...
<body>
  ...
  <input type="text" class="form-control" placeholder="关键字" id="competitionkey" name="competitionkey">
     <span class="input-group-btn">
        ...
        <h4 class="text-info">竞赛列表</h4>
        <?php if(is_array($competition)): $i = 0; $__LIST__ = $competition;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="classlist">
        <h5 class="classname text-primary"><?php echo ($vo["name"]); ?><small>类别：<?php echo ($vo["type"]); ?></small></h5>
        <h6 class="classtime">竞赛时间：<?php echo ($vo["time"]); ?></h6>
        <h6 class="classschool">校区：<?php echo ($vo["school"]); ?></h6>
        <h5 class="classprice text-warning">价格：<?php echo ($vo["price"]); ?>元<div class="btn-group classop"><button class="btn btn-warning btn-xs addtoshopcar" value="<?php echo ($vo["id"]); ?>">加入购物车</button><button class="btn btn-default btn-xs detail" value="<?php echo ($vo["id"]); ?>">竞赛详情</button></div></h5>
        
    </div><?php endforeach; endif; else: echo "" ;endif; ?>
    
     <div class="clear"></div>
 <ul class="pagination">      
 <?php $__FOR_START_496768091__=1;$__FOR_END_496768091__=$pagenumber;for($i=$__FOR_START_496768091__;$i < $__FOR_END_496768091__;$i+=1){ ?><li><a href="/jiayi/index.php/Home/User/buycompetition/page/<?php echo ($i); ?>/searchkey/<?php echo ($searchkey); ?>/"><?php echo ($i); ?></a></li><?php } ?>
   </ul>
    ...
    if (data.state==1) {
       ...
       $("#search").click(function(){
        var key=$("#competitionkey").val();
         window.location.href="/jiayi/index.php/Home/User/buycompetition/searchkey/"+key; 
       })
